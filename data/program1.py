...
class Pig_blue(pygame.sprite.Sprite):
    ...
    def update(self, types, *args):
        ...
        if self.life and types == 2 and SELECTED and self.select:
            ...
            if LEN <= RADIUS * 2.5:
                self.vector = (args[0][0] - self.rect.x - RADIUS, args[0][1] - RADIUS - self.rect.y)
            elif RADIUS * 2.5 > LEN:
                self.vector = ((self.rect.x + RADIUS) + 2.5 * RADIUS * (args[0][0] - self.rect.x - RADIUS) / LEN,
                               (self.rect.y + RADIUS) + 2.5 * RADIUS * (args[0][1] - self.rect.y - RADIUS) / LEN)
            pygame.draw.line(screen, (237, 118, 14), [self.rect.x + RADIUS, self.rect.y + RADIUS], [self.vector[0], self.vector[1]], 4)
        if self.life and types == 0 and (
                self.rect.x > 1385 or self.rect.x < 525 or self.rect.y < 85 or self.rect.y > 885):
            self.rect.x = WIDTH
            self.rect.y = HEIGHT
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            if event.type == pygame.MOUSEMOTION and TURN_PLAYER == 1 and SELECTED:
